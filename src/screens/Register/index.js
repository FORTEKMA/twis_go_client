...
import { View, Text, TextInput, TouchableOpacity, ScrollView, useWindowDimensions, Linking, Alert, ActivityIndicator } from 'react-native';
...
const Register = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
   ...
   .catch(error=>{
    ...
    } else if (error.response?.data?.error?.message === 'Phone number already exists') {
      setErrors({
        ...errors, 
  ...
  return (
   ...
   <TouchableOpacity onPress={() => navigation.goBack()} style={{ alignSelf: 'flex-start', marginLeft: 16, marginTop: 16, marginBottom: 8 }}>
    ...
    <ScrollView  style={{paddingHorizontal:10,paddingBottom:30}}>
      ...
      <View style={loginStyles.header}>
        <Text style={loginStyles.headerTitle}>{t('register.createAccount')}</Text>
      </View>
      <View style={loginStyles.formContainer}>
        <View style={loginStyles.inputContainer}>
          <Text style={{ fontWeight: '600', color: '#222', fontSize: 15, marginBottom: 6 }}>{t('register.name')}</Text>
          <TextInput
          ...
          />
          {errors.name && <Text style={loginStyles.errorText}>{errors.name}</Text>}
        </View>
        <View style={loginStyles.inputContainer}>
          <Text style={{ fontWeight: '600', color: '#222', fontSize: 15, marginBottom: 6 }}>{t('register.phone')}</Text>
          <View style={[loginStyles.passwordInputWrapper, errors.phone && loginStyles.inputError, { marginBottom: 0 }]}> 
            <PhoneInput
              ...
              style={{ flex: 1, fontSize: 16, color: '#222', paddingVertical: 5, paddingLeft: 18 }}
          ...
          {errors.phone && <Text style={loginStyles.errorText}>{errors.phone}</Text>}
        </View>
        <CountryPicker
          withFilter
          withFlag
        ...
        />
        <View style={loginStyles.inputContainer}>
          <Text style={{ fontWeight: '600', color: '#222', fontSize: 15, marginBottom: 6 }}>{t('register.email')}</Text>
          <TextInput
        ...
        <View style={loginStyles.inputContainer}>
          <Text style={{ fontWeight: '600', color: '#222', fontSize: 15, marginBottom: 6 }}>{t('register.password')}</Text>
        ...
        <Text style={{ color: '#8391A1', fontSize: 13, marginTop: 8, marginBottom: 16 }}>
          {t('register.terms1')}
          <Text style={{ color: PRIMARY_COLOR }} onPress={() => Linking.openURL('https://your-terms-url.com')}>{t('register.termsOfService')}</Text>
          {t('register.terms2')}
        </Text>
        <TouchableOpacity
          style={[loginStyles.btn, isLoading && loginStyles.btnDisabled, { backgroundColor: PRIMARY_COLOR }]}
        ...
        >
          {isLoading ? (
            ...
            <Text style={[loginStyles.btnText, { color: '#222' }]}>{t('register.signup')}</Text>
          )}
        </TouchableOpacity>
        ...
        <View style={{ alignItems: 'center', marginTop: 24 }}>
          <Text style={{ color: '#8391A1', fontSize: 14 }}>
            ...
            <Text style={{ color: PRIMARY_COLOR, fontWeight: '700' }} onPress={() => navigation.navigate('login')}>{t('register.signInHere')}</Text>
          </Text>
        </View>
      </View>
